...
class TargDelegate: NSObject {
    ...
    override func awakeFromNib() {
        ...
        NotificationCenter.defaultCenter().addObserver(self, selector: "boundsDidChange:", name: NSViewBoundsDidChangeNotification, object: targetScroll.contentView)
        NotificationCenter.defaultCenter().addObserver(self, selector: "boundsDidChange:", name:  NSViewFrameDidChangeNotification, object: targetClip)
        settings = UserDefaults.standard;  // Just in case this needs to be done after nib awake
        targetDelegate.targetFontSize = settings.doubleForKey(globals.targetSize)
        targetDelegate.targetFont = settings.string(forKey:globals.targetFont)!
    }
    ...
    @IBAction func doThing(sender: AnyObject) {
        ...
        let tsettingsURL = URL.fileURLWithPath( "/Users/WRE/DropBox/Amplify4/tsettings.plist")
//        let didit = tsettings.writeToURL(tsettingsURL!, atomically: true)
     ...
     func boundsDidChange (notification: Notification){
    ...
    func cleanupTarget() {
        ...
        let endHeader = settings.string(forKey:globals.endHeader)!
    ...
    func textViewDidChangeSelection(aNotification: Notification) {
    ...
    func textDidEndEditing(aNotification: Notification) {
        self.cleanupTarget()
    }
    
    func textDidChange(aNotification: Notification) {
    ...
    @IBAction func setBaseNumbers(sender: AnyObject) {
        ...
        let ehrange = targetString.rangeOfString(settings.string(forKey:globals.endHeader)!)
        ...
        if let maybeFont = NSFont(name: settings.string(forKey:globals.targetFont)!, size: CGFloat(settings.doubleForKey(globals.targetSize))) {
            theFont = maybeFont
        }
