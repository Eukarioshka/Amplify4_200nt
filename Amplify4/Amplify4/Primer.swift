...
class Primer : NSObject {
    ...
    func calcTm() -> Double {
        ...
        let entropy = settings.array(forKey:globals.entropy) as! [[Double]]
        let enthalpy = settings.array(forKey:globals.enthalpy) as! [[Double]]
    ...
    func calcZ() -> Bool {
        ...
        let settings = UserDefaults.standard
        let matchWeights = settings.array(forKey:globals.matchWeights)! as! [Int]
        let pairScores = settings.array(forKey:globals.pairScores)! as! [[Int]]
        let seqChar = [Character](seq.uppercaseString)
        var seqd = [Int]()
        var seqc = [Int]()
        let n = min(settings.integerForKey(globals.effectivePrimer), seqChar.count)
        ...
        maxStab *= (settings.array(forKey:globals.runWeights)! as! [Int])[n-1]
